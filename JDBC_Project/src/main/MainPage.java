...
public void initiate_db_conn()
{
	try
	{
		...
		// Specify the DB Name
		String URL="jdbc:mysql://localhost:3306/BEng_Assign?serverTimezone=" + TimeZone.getDefault().getID();
...
public void actionPerformed(ActionEvent e) {
	...
	else if (target == btnDelete)
	{
		...
		try
		{
			String updateTemp ="DELETE FROM players WHERE id = "+id.getText()+";"; 
			stmt.executeUpdate(updateTemp);
	...
	{	 	
		try
		{ 			
			String updateTemp ="UPDATE players SET " +
...
private void writeToFile(ResultSet rs){
	try{
		...
		FileWriter outputFile = new FileWriter("Assignment.csv");
